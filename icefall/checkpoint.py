...
def load_checkpoint(
	...
	emb_name = "encoder.emb.weight"
	def expand_emb(emb_weight: torch.Tensor, vocab_size: int):
		if emb_weight.shape[0] == vocab_size: return emb_weight
		logging.info(f"Expand tokens from {emb_weight.shape[0]} to {vocab_size}")
		ret = torch.zeros((vocab_size, emb_weight.shape[1]), dtype=emb_weight.dtype)
		ret[:emb_weight.shape[0],:] = emb_weight
		return ret
	def process_emb_layer(ckpt_emb: torch.Tensor):
		if ckpt_emb.shape == model.encoder.emb.weight.shape: return ckpt_emb
		assert safety_add_tokens, f"emb shape({ckpt_emb.shape} vs {model.encoder.emb.weight.shape}) doesn't match and safety_add_tokens is off! please make sure your tokens is from pretrained ckpt and turn on safety_add_tokens option."
		logging.info("Loading checkpoint from different token counts")
		source_emb_shape = ckpt_emb.shape
		target_emb_shape = model.encoder.emb.weight.shape
		assert source_emb_shape[1] == target_emb_shape[1], f"Embedding size({source_emb_shape[1]} vs {target_emb_shape[1]}) must match!"
		return expand_emb(ckpt_emb, target_emb_shape[0])
	if next(iter(checkpoint["model"])).startswith("module."):
		...
		for key in dst_state_dict.keys():
			...
			data = src_state_dict.pop(src_key)
			if key == emb_name: data = process_emb_layer(data)
			dst_state_dict[key] = data
		assert len(src_state_dict) == 0
		model.load_state_dict(dst_state_dict, strict=strict)
	else:
		checkpoint["model"][emb_name] = process_emb_layer(checkpoint["model"][emb_name])
